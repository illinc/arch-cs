...
% $arg$ "--- регистр или переменная в~памяти
\\\hline
imul arg, dest & Умножение $\usecopeq dest *= arg ~~(dest = dest\cdot arg)$
\\\hline
imul imm, arg, dest & Знаковое умножение $dest = imm\cdot arg$
% 
% ($imm$ "--- константа)
\\\hline

div arg & 
% Беззнаковое деление с~остатком %$A = (D{:}A)/arg$. % или $A/arg$.
\begin{tabular}{@{}l@{}}
...
shr src, dest & Беззнаковое деление  $\usecopeq {dest}/={2^{src}}$ (остаток не вычисляется)
\\\hline
sar src, dest & Знаковое деление $\usecopeq dest/=2^{src}$ (остаток не вычисляется)
\\\hline
\begin{tabular}{@{}l@{}}
shl src, dest 
\\
sal src, dest
\end{tabular}
& Умножение  $\usecopeq dest *= 2^{src}$ %(если результат умножения помещается в~dest%. Если не помещается, старшая часть теряется
% )
\\\hline
