...
% \end{tabular}
lea $\delta$(r1,r2,$\sigma$), dreg
& Вычисление эффективного адреса часто используется для~расчёта  линейной комбинации
$dreg = r1 + \sigma\cdot r2 + \delta$ 
% (не изменяет флагов)

$dreg, r1, r2$ "---  регистры, 	$\sigma$ "--- 1, 2, 4 или 8, $\delta$ "--- константа 
% 
...
\\\hline
sar times, dest & Знаковое деление $\usecopeq dest/=2^{times}$ (остаток не вычисляется и~предполагается неотрицательным, см. раздел~\ref{sec:digits:sar}
)
\\\hline
\begin{tabular}{@{}l@{}}
shl times, dest 
