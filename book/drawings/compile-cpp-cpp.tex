\tikzstyle{blockarrow}	= [-latex',thick,draw=clCompileArrow]
...
\tikzstyle{file}	= [block, compilefile]
\tikzstyle{stage}	= [block, compilestage]
...
\node[file,right= of prog1_cpp-|cpp_egde,on chain=S] (prog2_S) {prog3.cpp};
\node[stage,on chain=S] (S_preprocess) {Препроцессор};
\node[file, on chain=S] (prog2_pp) {Временный файл (stdout)};
\node[stage,on chain=S]  (compiler2) {Компилятор};
\node[file,on chain=S]  {prog3.s};
\node[stage,on chain=S,right= of cpp_asm-|cpp_egde]  (S_asm) {Ассемблер};
\node[file,on chain=S] (prog2_o) {prog3.o};
...
\node[rectangle,fit=(compiler) (mangling) (compiler2), compilemanglingback]  {};
