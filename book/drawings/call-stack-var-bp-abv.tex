...
\foreach \sp/\bp in {1/-1, 2/-1, 5/2, 2/2, 1/-1}
{
...
\tikzstyle{mem}	= [block,  fill=clMemoryLight]
  \node[mem,solidchaincell,on chain=Mem, label=left:$f:$] (c1) {push $bp$};
  \node[mem,solidchaincell,on chain=Mem] (c2) {$bp=sp$};
  \node[mem,solidchaincell,on chain=Mem]  {$sp = sp - size$};
  \node[mem,solidchaincell,on chain=Mem] (c3) {$\ldots$};
  \node[mem,solidchaincell,on chain=Mem]  {$sp=bp$};
  \node[mem,solidchaincell,on chain=Mem] (c4) {pop $bp$};
  \node[mem,solidchaincell,on chain=Mem] (c5)  {$ret$};
 
  \node[reg, right= of s\sp]  (sp) {$sp$};
  \path[blockarrow] (sp) -- (s\sp);  
  
   \xintifboolexpr{\bp > 0}
    {\node[reg, right= of s\bp]  (bp) {$bp$}; \path[blockarrow] (bp) -- (s\bp)}
    {}
  ;  
  
   \xintifboolexpr{\bp = \sp}
    {\node[reg, right= of s\bp, text width=9ex]  (bp) {$sp = bp$}}
    {}
  ;  
...
% %   \path[blockarrow] (bp) -- (s\sp);  
  ...
  \node[on chain=Mem]   {\normalfont\normalsize\asbuk{figletter})\stepcounter{figletter}};
}
