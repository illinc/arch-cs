...
\item измените тип с~int на  на double, single, long double% (деление (\lstinline!/!) "--- вещественное, для расчёта остатка (\lstinline!%!) усекайте вещественные переменные до int)
.
\end{itemize}
...
\item
Оформите вычисления из задания \reflocal{task:dev:int:a} как целую функцию от целого аргумента.
...
\section{Отладка программы на языке C++}
...
Рассмотрим инструменты отладки на примере  среды Qt Creator (рис.~\reflocal{ris:qtc}).
...
Обычно точки останова связаны с файлом исходных кодов и строкой или началом функции (рис.~\reflocal{ris:qtc-breakpoint}).
\begin{illustration}
\includegraphics[width=\wideImageWidth]{qtc-breakpoint}

\caption{Точка останова в~коде}
\labellocal{ris:qtc-breakpoint}
\end{illustration}
...
Окно «\insertsubsubsection»  может быть использовано для изменения содержимого переменной простого типа данных (такого как int или float) во время останова программы (рис.~\reflocal{ris:qtc-watches}).
\begin{illustration}
\includegraphics[width=\wideImageWidth]{qtc-watches}
\caption{\insertsubsubsection{} Qt Creator}
\labellocal{ris:qtc-watches}
\end{illustration}
...
Как обозреватель, так и~редактор памяти можно открыть для просмотра конкретной переменной, используя контекстное меню переменной в~окне «Локальные и~наблюдаемые переменные» (рис.~\reflocal{ris:qtc-memory}), при этом подсвечивается не только начало, но и~размер объекта.
\begin{illustration}
\includegraphics[width=\linewidth]{qtc-memory}
\caption{\insertsubsubsection{} Qt Creator}
\labellocal{ris:qtc-memory}
\end{illustration}
...
Окно «\insertsubsubsection»  отображает дизассемблированный код для текущей функции (рис.~\reflocal{ris:qtc-disasm}).
...
В~частности, в~зависимости от используемого отладчика и~его настроек, может различаться используемый синтаксис.
...
Рис.~\reflocal{ris:cb-disasm} показывает окно дизассемблера среды Code::Blocks  с~включённой опцией $Mixed~mode$ (синтаксис AT\&T).
\begin{illustration}
\includegraphics[width=\linewidth]{cb-disasm}
\caption{\insertsubsubsection{} (Code::Blocks)}
\labellocal{ris:cb-disasm}
\end{illustration}
...
\caption{\insertsubsubsection{} (Qt Creator)}
\labellocal{ris:qtc-reg}
\end{illustration}



Окно «\insertsubsubsection»  отображает текущее содержимое регистров (рис.~\reflocal{ris:qtc-reg}).
