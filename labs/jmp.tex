...
(
  ...
  "cmovll %[Zero], %[Y]\n"
  : [Y]"=r"(y)
  : [X]"rm"(x),"[Y]"(y),[Zero]"rm"(0)
  : "cc"
);
\end{lstlisting}
Здесь параметр \lstinline![Y]! объявлен как входной и~выходной одновременно, хотя, казалось бы, по сути он является только выходным.
...
Если не указать \lstinline![Y]! в~списке входных и~он будет совмещён с~\lstinline![Zero]!, мы всегда получим на выходе $y=1$.
